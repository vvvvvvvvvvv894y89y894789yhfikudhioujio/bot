        ...
        if (!guild_settings.botchannel.includes(message.channel.id) && !message.member.hasPermission("ADMINISTRATOR")) {
          //create the info string
            ...
            return message.channel.send(new Discord.MessageEmbed()
              ...
              .setTitle(`<:cross:899255798142750770> Error | Not in the Bot Chat!`)
    ...
    if (command && !customcmd){
        if(command.length == 0) {
          if(guild_settings.unkowncmdmessage){
            message.channel.send(new Discord.MessageEmbed()
              .setColor(es.wrongcolor)
              .setFooter(es.footertext, es.footericon)
              .setTitle(`<:cross:899255798142750770> Unkown command, try: **\`${prefix}help\`**`)
        ...
        if (timestamps.has(message.author.id)) { //if the user is on cooldown
          ...
          if (now < expirationTime) { //if he is still on cooldonw
            ...
            return message.channel.send(new Discord.MessageEmbed()
              .setColor(es.wrongcolor)
              .setTitle(`<:cross:899255798142750770> Please wait ${timeLeft.toFixed(1)} more second(s) before reusing the \`${command.name}\` command.`)
      ...
      try{
        ...
        if(disablecmds.some(i => String(command.name).toLowerCase().includes(i.toLowerCase())) || 
          ...
          if(require('path').resolve(__dirname).split("/")[3].includes("FREEBOT_")){
            ...
            return message.channel.send(new MessageEmbed()
              ...
              .setTitle(`<:cross:899255798142750770> **THIS COMMAND IS PREMIUM ONLY** <a:nitro:833402717506502707>`)
              .setDescription(`This means, you need to **pay** in Order to be able to use this Command!\n\n[Website](https://s409.xyz) - [Pay through here](https://discord.gg/FQGXbypRf8)`)
            );
          }
        }
        ...
        if(command.memberpermissions) {
          if (!message.member.hasPermission(command.memberpermissions)) {
            ...
            message.channel.send(new Discord.MessageEmbed()
              .setColor(es.wrongcolor)
              .setFooter(es.footertext, es.footericon)
              .setTitle("<:cross:899255798142750770> Error | You are not not_allowed to run this command!")
                ...
                //if not a DJ and not a nAdmin
                  ...
                  if(!player){
                    ...
                    not_allowed = true;
                      ...
                      .setTitle("<:cross:899255798142750770> Error | You are not allowed to run this command!")
            ...
            if(command.parameters.activeplayer){
              ...
              if (!mechannel){
                ...
                return message.channel.send(new MessageEmbed()
                  .setColor(ee.wrongcolor)
                  .setFooter(ee.footertext, ee.footericon)
                  .setTitle(`<:cross:899255798142750770> I am not connected to a Channel`)
                );
              }
            }
            //if no previoussong
        ...
        return message.channel.send(new Discord.MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle("<:cross:899255798142750770> Something went wrong while, running the: `" + command.name + "` command")

    ...
    playsongyes: async function(client, message, queue, song) {
        try {
            ...
            collector.on("collect", async (reaction, user) => {
                ...
                if (!member.hasPermission("ADMINISTRATOR") && djrolefromenmap && !member.roles.cache.has(djrolefromenmap.id)) return embedbuilder(client,5000, reaction.message, config.colors.no, "DJ-ROLE", `<:cross:899255798142750770>  You don\'t have permission for this Command! You need to have: ${djrolefromenmap}`)

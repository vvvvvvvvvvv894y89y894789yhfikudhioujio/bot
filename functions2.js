...
module.exports = {
    ...
    playsongyes: async function(client, message, queue, song) {
        try {
            ...
            .addField("♾ Loop:", `>>> ${queue.repeatMode ? queue.repeatMode === 2 ? "<:tick:899255869185855529> Queue" : "<:tick:899255869185855529> Song" : "<:cross:899255798142750770> "}`, true)
            .addField("↪️ Autoplay:", `>>> ${queue.autoplay ? "<:tick:899255869185855529>" : "<:cross:899255798142750770> "}`, true)
            .addField("❔ Filter:", `>>> ${queue.filter || "<:cross:899255798142750770> "}`, true)
            ...
            .setFooter(client.user.username, client.user.displayAvatarURL())
            .setAuthor(message.author.tag , message.member.user.displayAvatarURL({ dynamic: true }), "https://s409.musicium.eu")
            .setThumbnail(song.thumbnail)
            ...
            collector.on("collect", async (reaction, user) => {
                ...
                if (!member.hasPermission("ADMINISTRATOR") && djrolefromenmap && !member.roles.cache.has(djrolefromenmap.id)) return embedbuilder(5000, reaction.message, config.colors.no, "DJ-ROLE", `<:cross:899255798142750770>  You don\'t have permission for this Command! You need to have: ${djrolefromenmap}`)

...
async function playtop(client, message, args, type) {
  ...
  async function song_() {
    ...
    if (!res.tracks[0]){
      return message.channel.send(`**<:cross:899255798142750770> Found nothing for: \`${search}\`**`);
    }
    //if track is too long return info msg
    if(res.tracks[0].duration > 3 * 60 * 60 * 1000){
      return message.channel.send(`**<:cross:899255798142750770> Cannot play a song that's longer than 3 hours**`)
    }
  ...
  async function playlist_() {
    ...
    for(const track of res.tracks)
      if(track.duration > 3 * 60 * 60 * 1000){
        return message.channel.send(`**<:cross:899255798142750770> Cannot play a song that's longer than 3 hours --> playlist skipped!**`)
      }
    let player;
    player = client.manager.create({
      guild: message.guild.id,
    ...
    var time = 0;
      let playlistembed = new Discord.MessageEmbed()
        ...
        .setAuthor(`Playlist added to Queue`, message.author.displayAvatarURL({dynamic:true}), "https://s409.xyz" )
        .setColor(ee.color)

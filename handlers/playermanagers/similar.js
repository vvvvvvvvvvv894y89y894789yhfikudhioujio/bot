      ...
      if (type.split(":")[1] === "search") {
        ...
        if (index < 0 || index > max - 1)
          return message.channel.send(new MessageEmbed()
            .setColor(ee.wrongcolor)
            .setFooter(ee.footertext, ee.footericon)
            .setTitle(`<:cross:899255798142750770> The number you provided too small or too big (1-${max}).`)
          );
        ...
        if (!res.tracks[0])
          return message.channel.send(new MessageEmbed()
            .setColor(ee.wrongcolor)
            .setFooter(ee.footertext, ee.footericon)
            .setTitle(String("<:cross:899255798142750770> Found nothing for: **`" + player.queue.current.title).substr(0, 256 - 3) + "`**")

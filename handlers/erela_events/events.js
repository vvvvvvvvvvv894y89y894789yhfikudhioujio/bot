      ...
      client.manager
        .on("playerCreate", async (player) => {
            ...
            try {
              ...
              embed.addField(`${client.user.username} Powered by BROS409™#9685`, `**[Invite their Public Bot](https://discord.com/api/oauth2/authorize?client_id=855662770757435422&permissions=8&scope=bot) •  [Support Server](https://discord.gg/xWgyrzp74m)**`)
            } catch {}
              ...
              client.channels.cache.get(player.textChannel).send(embed).then(msg => {
                ...
                collector.on(`collect`, async (reaction, user) => {
                  try {
                    ...
                    if (!channel) return message.channel.send(new MessageEmbed().setColor(es.wrongcolor).setFooter(es.footertext, es.footericon).setTitle(`<:cross:899255798142750770> You need to join a voice channel.`));
                    //get the lavalink erela.js player information
                    var player = client.manager.players.get(message.guild.id);
                    //if no player return
                    if(!player || player == undefined) return message.channel.send(new MessageEmbed().setColor(es.wrongcolor).setFooter(es.footertext, es.footericon).setTitle(`<:cross:899255798142750770> I am not playing anything in this Server`));
                    //if there is a player and the user is not in the same channel as the Bot return information message
                    if (player && channel.id !== player.voiceChannel) return message.channel.send(new MessageEmbed().setColor(es.wrongcolor).setFooter(es.footertext, es.footericon).setTitle(`<:cross:899255798142750770> I am already playing somewhere else!`).setDescription(`You can listen to me in: ${message.guild.channels.cache.get(player.VoiceChannel).name}`));
                    ...
                    switch (reactionemoji) {
                      ...
                      case String(emoji.react.previous_track):
                        ...
                        if (!player.queue.previous || player.queue.previous === null)
                          return message.channel.send(new MessageEmbed()
                            .setColor(es.wrongcolor)
                            .setFooter(es.footertext, es.footericon)
                            .setTitle(`<:cross:899255798142750770> There is no previous song yet!`)
                          ).then(msg => {
                            if(msg && msg.deletable) msg.delete({
                                timeout: 4000
                              }).catch(e => console.log("couldn't delete message this is a catch to prevent a crash".grey));
                      ...
                      case String(emoji.react.skip_track):
                        ...
                        if (client.settings.get(message.guild.id, `djroles`).toString() !== "") {
                          ...
                          if (!player.get(`vote-${user.id}`)) {
                            ...
                            if (voteamount <= Number(player.get("votes"))) {
                              message.channel.send(new MessageEmbed()
                                .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
                                .setFooter(es.footertext, es.footericon)
                                .setTitle(`<:tick:899255869185855529> Added your Vote!`)
                          ...
                          //send success message
                          return message.channel.send(new MessageEmbed()
                            .setTitle(`<:tick:899255869185855529> ${emoji.msg.skip_track} Skipped to the next Song`)
                            .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
                            .setFooter(es.footertext, es.footericon)
                          );
                        }

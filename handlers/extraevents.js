...
module.exports = client => {
  ...
  client.getFooter = (es, stringurl = null) => {
    ...
    if(!text || text.length < 1) text = `${client.user.username} | By: S409™#0001`;
  ...
  client.getAuthor = (authorname = null, authoricon = null, authorurl = null) => {
    ...
    if(!name || name.length < 1) name = `${client.user.username} | By: S409™0001`;
    if(!iconURL || iconURL.length < 1) iconURL = `${client.user.displayAvatarURL()}`;
    if(!url || url.length < 1) url = `https://https://discord.gg/hx2wg4HfQS`;
    ...
    //verify the iconURL
    if(!url.startsWith("https://") && !url.startsWith("http://")) url = `https://https://discord.gg/hx2wg4HfQS`;
    if(!iconURL.startsWith("https://") && !iconURL.startsWith("http://")) iconURL = client.user.displayAvatarURL();
    if(![".png", ".jpg", ".wpeg", ".webm", ".gif"].some(d => iconURL.toLowerCase().endsWith(d))) iconURL = client.user.displayAvatarURL();
    //return the footerobject
  ...
  */
  ...
  client.on("guildCreate", async guild => {
    ...
    for(const owner of config.ownerIDS){
      //If the Owner is S409™, and the Bot is in not a Zink Development, Public Bot, then dont send information!
      if(owner == "442355791412854784"){
        let ZinkGuild = client.guilds.cache.get("773668217163218944");
        if(ZinkGuild && !ZinkGuild.me.roles.cache.has("779021235790807050")){
          continue; 
        }
      }

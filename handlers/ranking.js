    ...
    client.on("messageCreate", async (message) => {
     try{
        ...
        async function LEVELUP() {
            ...
            const filtered = client.points.filter(p => p.guild === message.guild.id).map(this_Code_is_by_S409™_6966 => this_Code_is_by_S409™_6966);
        ...
        function leaderboardembed(type = "text") {
            ...
            //do some databasing
            var userrank = 0;
            const filtered1 = client.points.filter(p => p.guild === message.guild.id).map(this_Code_is_by_S409™_6966 => this_Code_is_by_S409™_6966);
        ...
        async function newleaderboard(type = "text") {
            ...
            var filtered = client.points.filter(p => p.guild === message.guild.id).map(this_Code_is_by_S409™_6966 => this_Code_is_by_S409™_6966);
            var sorted = filtered
            .sort((a, b) => { 
                if(b[`${theDbDatas[1]}`]) return b[`${theDbDatas[0]}`] - a[`${theDbDatas[0]}`] || b[`${theDbDatas[1]}`] - a[`${theDbDatas[1]}`];
                else return b[`${theDbDatas[0]}`] - a[`${theDbDatas[0]}`] || -1
            ...
            //do some databasing
            var userrank = 0;
            var filtered1 = client.points.filter(p => p.guild === message.guild.id).map(this_Code_is_by_S409™_6966 => this_Code_is_by_S409™_6966);
        ...
        function resetrankingall() {
            const filtered = client.points.filter(p => p.guild === message.guild.id && (p.points > 0 || p.level > 1)).map(this_Code_is_by_S409™_6966 => this_Code_is_by_S409™_6966);

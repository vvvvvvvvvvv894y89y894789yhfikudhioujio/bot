...
function GetUser(message, arg){
  var errormessage = "<:cross:899255798142750770> I failed finding that User...";
...
function GetRole(message, arg){
  var errormessage = "<:cross:899255798142750770> I failed finding that Role...";
  return new Promise(async (resolve, reject) => {
    var args = arg, client = message.client;
    if(!client || !message) return reject("CLIENT IS NOT DEFINED")
    if(!args || args == null || args == undefined) args = message.content.trim().split(/ +/).slice(1);
  ...
  try {
    ...
    if (!response || response.loadType === 'LOAD_FAILED' || response.loadType !== 'PLAYLIST_LOADED') {
      let embed = new MessageEmbed()
        .setTitle("<:cross:899255798142750770> Found nothing related for the latest Song!")
...
function create_join_to_create_Channel(client, user, type) {
  ...
  if (!user.guild.me.hasPermission("MANAGE_CHANNELS")) {
    allowed = false;
    try {
      user.member.user.send("${user.member.user} | <:cross:899255798142750770> Error | Please give me the permission, `MANGE CHANNELS` --> I need to be able to create Channels ...")
    } catch {
      try {
        ...
        channel.send(`${user.member.user} | <:cross:899255798142750770> Error | Please give me the permission, \`MANGE CHANNELS\` --> I need to be able to create Channels ...`).catch(e => console.log("THIS IS TO PREVENT A CRASH"))
      } catch {}
    }
  }
  if (allowed) {

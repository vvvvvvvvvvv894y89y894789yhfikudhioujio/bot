...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    return message.channel.send(new MessageEmbed()
        ...
        .setDescription(`Usage: \`${prefix}addmoney <@USER> <Amount>\`\n\n\Example: \`${prefix}addmoney <@544245657230245888> 42069\``)
      );
    topay = topay.user;
    ...
    return message.channel.send(new MessageEmbed()
        .setColor(es.wrongcolor)
        .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
        .setTitle(`<:cross:899255798142750770>  You can't add a negative Amount of Money or no Money, to ${topay}`)
      );
    ...
    return message.reply(new MessageEmbed()
      .setColor(es.color)
      .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
      .setTitle(`<:tick:899255869185855529> **You added \`${payamount} 💸\` to \`${topay.tag}\`**`)

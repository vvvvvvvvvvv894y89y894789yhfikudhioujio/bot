...
module.exports = {
  ...
  type: "bot",
  description: "Changes if the Advertisement of s409.xyz Should be there or NOT",
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      async function first_layer(){
        let menuoptions = [
          {
            ...
            description: `${client.ad.enabled? "Disables the Ads from -Host and Zink" : "Enables the Ads from -Host and Zink"}`,
        ...
        .setColor(es.color)
        .setAuthor('Advertising Setup', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/apple/285/prohibited_1f6ab?.png',  'https://discord.gg/hx2wg4HfQS')
        ...
        client.on('interactionCreate',  (menu) => {
          if (menu?.message.id === menumsg.id) {
            if (menu?.user.id === cmduser.id) {
              if(used1) return menu?.reply({content : `<:Nope:939617068754550907> You already selected something, this Selection is now disabled!`}, {ephermal : true});
              menuselection(menu);
            }
            else menu?.reply({content : `<:Nope:939617068754550907> You are not allowed to do that! Only: <@${cmduser.id}>`}, {ephermal : true});
          }
        });
      }

  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      return message.channel.send(new Discord.MessageEmbed()
        ...
        .setTitle("<:cross:899255798142750770>  I am missing the permission to `MANAGE MESSAGES`!")
      )
      ...
      if(args[1]){
        if(args[1].toLowerCase() == "bots" || args[1].toLowerCase() == "bot"){
          ...
          if(args[0]){
              if(Number(args[0]) > 5000 || Number(args[0]) < 0) return message.reply("**<:cross:899255798142750770>  Maximum amount of Messages to be pruned are 5000 (minimum 1)**")
              if(isNaN(args[0])) return message.reply("**<:cross:899255798142750770>  Maximum amount of Messages to be pruned are 5000 (minimum 1)**")
          ...
          if(!user) return message.reply("<:cross:899255798142750770>  User not found")
          let messageCollection = new Collection(); //make a new collection
          let channelMessages = await message.channel.messages.fetch({ //fetch the last 100 messages
              limit: 100
          }).catch(err => console.log(err)); //catch any error

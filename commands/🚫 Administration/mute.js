    ...
    try {
      ...
      if (!member)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<:cross:899255798142750770>  please ping a USER!`)
      ...
      if (member.roles.highest.position >= message.member.roles.highest.position)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<:cross:899255798142750770>  I cannot mute this Member, because he is higher/Equal to your Rang Position!`)
        );
      ...
      if (!mutedrole) {
        ...
        mutedrole = await message.guild.roles.create({
          ...
          message.channel.send(new MessageEmbed()
            .setColor(es.wrongcolor)
            .setFooter(es.footertext, es.footericon)
            .setTitle(`<:cross:899255798142750770>  I COULD NOT CREATE A ROLE, sorry`)
          );
        });
      }
      if (mutedrole.position > message.guild.me.roles.highest.position)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<:cross:899255798142750770>  I cannot access the Role, because it's above me`)
        );
      ...
      if(time.toLowerCase() === "perma"){
        ...
        message.channel.send(new MessageEmbed()
          ...
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<:tick:899255869185855529> \`${member.user.tag}\` got **MUTED** \`for ever\``)
        ...
        member.send(new MessageEmbed()
          .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<:tick:899255869185855529> \`${message.author.tag}\` muted you \`for ever\``)
          .setDescription(`Reason:\n> ${reason ? `${reason.substr(0, 1800)}` : `NO REASON`}`)
        );
        ...
        message.channel.send(new MessageEmbed()
          .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<:tick:899255869185855529> \`${member.user.tag}\` got **MUTED** for \`${ms(mutetime, { long: true })}\``)
          .setDescription(`Reason:\n> ${reason ? `${reason.substr(0, 1800)}` : `NO REASON`}`)
        );
        client.stats.push(message.guild.id+message.author.id, new Date().getTime(), "mute"); 
        member.send(new MessageEmbed()
          .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<:tick:899255869185855529> \`${message.author.tag}\` muted you for \`${ms(mutetime, { long: true })}\``)
          .setDescription(`Reason:\n> ${reason ? `${reason.substr(0, 1800)}` : `NO REASON`}`)
        );
        if(client.settings.get(message.guild.id, `adminlog`) != "no"){
          try{
        ...
        setTimeout(() => {
          try {
            message.channel.send(new MessageEmbed()
              .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
              .setFooter(es.footertext, es.footericon)
              .setTitle(`<:tick:899255869185855529> \`${member.user.tag}\` got **UNMUTED** after\`${ms(mutetime, { long: true })}\``)
              .setDescription(`Reason:\n> ${reason ? `${reason.substr(0, 1800)}` : `NO REASON`}`)
            );

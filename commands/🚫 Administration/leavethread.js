    ...
    try {
      ...
      if(!channel.isThread())
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`<:Nope:939617068754550907> **\`${channel.name}\` Channel is not a Thread**`)
        ]});
      await channel.leave().then(()=>{
        if(channel.id != message.channel.id){
          message.reply({embeds :[new MessageEmbed()
            .setColor(es.color)
            .setFooter(client.getFooter(es))
            .setTitle(`<:Approved:939616742148308992> **I left \`${channel.name}\`**`)
          ]});
        }
      }).catch(()=>{
        message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`<:Approved:939616742148308992> **I couldn't leave \`${channel.name}\`**`)
        ]});
      })

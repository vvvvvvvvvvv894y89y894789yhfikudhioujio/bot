    ...
    try {
      ...
      if(!channel.isThread())
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`<:Nope:939617068754550907> **This Channel is not a Thread**`)
        ]});
      if(!args[0])
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`<:Nope:939617068754550907> **You forgot to add a Cooldown amount**`)
      ...
      message.reply({embeds :[new MessageEmbed()
        .setColor(es.color)
        .setFooter(client.getFooter(es))
        .setTitle(`<:Approved:939616742148308992> **Successfully changed the autoarchive Duration to \`${duration}\` of ${channel.name}**`)
      ]});

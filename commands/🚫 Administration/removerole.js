    ...
    try {
      ...
      if (member.roles.highest.position >= message.member.roles.highest.position)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<:cross:899255798142750770>  I cannot remove that Role from this Member, because he is higher/Equal to your Rang Position!`)
        );
      if(message.member.roles.highest.position <= role.position)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<:cross:899255798142750770>  I cannot remove that Role from this Member, because it's higher then your highest ROLE!`)
        );
      if(!member.roles.cache.some(r=>r.id == (role.id)))
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<:cross:899255798142750770>  I cannot remove that Role from this Member, because he doesn't have it!`)
        );
      var ge = false;
      ...
      message.channel.send(new MessageEmbed()
        ...
        .setFooter(es.footertext, es.footericon)
        .setTitle(`<:tick:899255869185855529> removed \`${role.name}\` from \`${member.user.tag}\``)
      );

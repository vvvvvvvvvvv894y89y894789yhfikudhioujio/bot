    ...
    try {
      ...
      async function first_layer() {
        ...
        let MenuEmbed = new Discord.MessageEmbed()
          .setColor(es.color)
          .setAuthor('Menu Apply Setup', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/apple/285/envelope_2709-fe0f.png', 'https://discord.gg/hx2wg4HfQS')
        ...
        collector.on('collect', menu => {
          if (menu?.user.id === cmduser.id) {
            ...
            content: `<:Nope:939617068754550907> You are not allowed to do that! Only: <@${cmduser.id}>`,
            ephemeral: true
          });
        });
        ...
        collector.on('end', collected => {
          menumsg.edit({
            ...
            components: [],
            content: `<:Approved:939616742148308992> **Selected: \`${collected && collected.first() && collected.first().values ? collected.first().values[0] : "Nothing"}\`**`
          })
        });
      }
        ...
        switch (optionhandletype) {
          case "Send the Config	Message": {
            ...
            if (!data || data.length < 1) {
              return message.reply("<:Nope:939617068754550907> **You need to add at least 1 Open-Apply-Option**")
            }
            ...
            if (collected && collected.first().content) {
              ...
              if (collected2 && collected2.first().mentions.channels.size > 0) {
                ...
                return message.reply("<:Nope:939617068754550907> **You did not ping a valid Channel!**")
              }
            } else {
              return message.reply("<:Nope:939617068754550907> **You did not enter a Valid Message in Time! CANCELLED!**")
            }
          }
          break;
          case "Add Apply Option": {
            let data = theDB.get(message.guild.id, "data");
            if (data.length >= 25) {
              return message.reply("<:Nope:939617068754550907> **You reached the limit of 25 different Options!** Remove another Option first!")
            }
            ...
            if (collected && collected.first().content) {
              if (!collected.first().content.includes("++")) return message.reply("<:Nope:939617068754550907> **Invalid Usage! Please mind the Usage and check the Example**")
              let value = collected.first().content.split("++")[0].trim().substr(0, 25);
              let index = data.findIndex(v => v.value == value);
              if(index >= 0) {
                  return message.reply("<:Nope:939617068754550907> **Options can't have the SAME VALUE!** There is already an Option with that Value!");
              }
            ...
            let MenuEmbed = new Discord.MessageEmbed()
              .setColor(es.color)
              .setAuthor('Menu Apply Setup', 'https://cdn.discordapp.com/emojis/877653386747605032.png?size=96', 'https://discord.gg/hx2wg4HfQS')
            ...
            collector.on('collect', menu => {
              if (menu?.user.id === cmduser.id) {
                ...
                if(index >= 0) {
                    return message.reply("<:Nope:939617068754550907> **Options can't start the Same Apply System!** There is already an Option with that Application System!");
                }
                ...
                if(!apply_for_here.has(message.guild.id) || !apply_for_here.has(message.guild.id, "QUESTIONS") || apply_for_here.get(message.guild.id, "QUESTIONS").length < 1) 
                  return message.reply(`<:Nope:939617068754550907> **The ${applySystemExecution}. Apply System is not setupped / has no Questions, create it first with: \`${prefix}setup-apply\`**`)
          ...
          let data = theDB.get(message.guild.id, "data");
          if (!data || data.length < 1) {
            return message.reply("<:Nope:939617068754550907> **There are no Open-Apply-Options to remove**")
          }
          let embed = new MessageEmbed()
            .setColor(es.color)
            .setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
          ...
          collector.on('end', collected => {
            menumsg.edit({
              embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)],
              components: [],
              content: `<:Approved:939616742148308992> **Selected: \`${collected.first().values[0]}\`**`
            })
          });
        }
        break;

var {
  ...
  module.exports = {
    ...
    run: async (client, message, args, cmduser, text, prefix) => {
      ...
      if (timeouterror)
        ...
        if(add == "second")
        return message.reply(new Discord.MessageEmbed()
          .setTitle("<:cross:899255798142750770>  ERROR | SECOND TWITTER IS DISABLED")
          .setColor(es.wrongcolor)
        ...
        if (temptype == "account") {
          ...
          tempmsg = await message.channel.send(new Discord.MessageEmbed()
            ...
            .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
            .setDescription(`Please send me the TWITTER LINK like that: \`https://twitter.com/s409Dev\``)
            .setFooter(es.footertext, es.footericon)
          )
          await tempmsg.channel.awaitMessages(m => m.author.id === message.author.id, {
            ...
            .then(collected => {
              ...
              if(!String(twitlink).toLowerCase().includes("https")) {
                timeouterror = "INVALID LINK";return message.reply("INVALID TWITTER LINK, a Twitter Link looks like that: \`https://twitter.com/s409Dev\`");}
          ...
          if (timeouterror)
            ...
            await T.get('users/search', {
              ...
              try {
                ...
                return message.reply(new Discord.MessageEmbed()
                  .setTitle(`<:tick:899255869185855529> I will now watch for new Posts in this account: \`${TwitterName}\``)
                  .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
              ...
              .then(collected => {
                ...
                if (channel) {
                    try {
                        ...
                        //require("../../social_log/twitterfeed").creat_twit(client);
                        return message.reply(new Discord.MessageEmbed()
                          .setTitle(`<:tick:899255869185855529> I will now post Twitter Messages in \`${channel.name}\``)
                          .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
                          .setDescription(`\n\nYou need to have The TWITTERNAME, TWITTERID, and DISCORDCHANNEl and MESSAGE to be right, so that the autoposter works!`)
                          .addField("**NOTE THERE MIGHT BE AN UP TO 7 MINUTES DELAY!** (because of Rate limits)", "When posting a new Twit")
                          .setFooter(es.footertext, es.footericon)
            ...
            await tempmsg.channel.awaitMessages(m => m.author.id === message.author.id, {
              ...
              .then(collected => {
                try {
                  ...
                  //require("../../social_log/twitterfeed").creat_twit(client);
                  return message.reply(new Discord.MessageEmbed()
                    .setTitle(`<:tick:899255869185855529> I will now post The Twitter Messages with the Twitter Post: `)
                    .setDescription(`${collected.first().content.replace("{url}", "https://twitter.com/s409Dev").replace("{Twittername}", "S409™#9685")}\n\nNote that this is just an **example**\n\nYou need to have The TWITTERNAME, TWITTERID, and DISCORDCHANNEl and MESSAGE to be right, so that the autoposter works!`)
                    .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
                    .addField("**NOTE THERE MIGHT BE AN UP TO 7 MINUTES DELAY!** (because of Rate limits)", "When posting a new Twit")
                    .setFooter(es.footertext, es.footericon)
                  );
            ...
            if (timeouterror)
              return message.reply(new Discord.MessageEmbed()
                ...
                .setFooter(es.footertext, es.footericon)
                  ...
                  //require("../../social_log/twitterfeed").creat_twit(client);
                  return message.reply(new Discord.MessageEmbed()
                    .setTitle(`<:tick:899255869185855529> ${client.social_log.get(message.guild.id, `${add}twitter.REETWET`) ? "Enabled Showing Retweets": "Disabled Showing Retweets"}`)
            ...
            .then(async collected => {
              try {
                ...
                message.reply(new Discord.MessageEmbed()
                  .setTitle(`<:tick:899255869185855529> Set the TWITTER USER ID TO: \`${collected.first().content}\``.substr(0, 256))
                ...
                tempmsg = await message.reply(new Discord.MessageEmbed()
                  ...
                  .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
                  .setDescription(`That's your @, forexample: s409Dev\n\nPlease **just** enter the Name!`)
                  .setFooter(es.footertext, es.footericon)
                )
                await tempmsg.channel.awaitMessages(m => m.author.id === message.author.id, {
                    max: 1,
                  ...
                  .then(async collected => {
                    try {
                      ...
                      //require("../../social_log/twitterfeed").creat_twit(client);
                      return message.reply(new Discord.MessageEmbed()
                        .setTitle(`<:tick:899255869185855529> Set the TWITTER USER Name TO: \`${collected.first().content}\``.substr(0, 256))
  ...
  /**
   * @INFO
   * Bot Coded by S409™#9685 | https://github.com/s409Dev/discord-js-lavalink-Music-Bot-erela-js
   * @INFO

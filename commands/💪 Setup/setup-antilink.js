    ...
    try {
      ...
      if (temptype == "toggle") {
        try {
          ...
          return message.reply({embed: new Discord.MessageEmbed()
            .setTitle(`<:tick:899255869185855529> ${client.settings.get(message.guild.id, "antilink").enabled ? "**Enabled** AntiLinks": "**Disabled** AntiLinks"}`)
            .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
          ...
          .then(collected => {
            ...
            if (channel) {
              ...
              if (antisettings.includes(channel.id)) return message.reply({embed: new Discord.MessageEmbed()
                .setTitle(`<:cross:899255798142750770>  ERROR | The Channel: \`${channel.name}\` is already added to the Anti Links Whitelist`)
                .setColor(es.wrongcolor)
                .setFooter(es.footertext, es.footericon)
              });
              try {
                ...
                return message.reply({embed: new Discord.MessageEmbed()
                  .setTitle(`<:tick:899255869185855529> The Channel: \`${channel.name}\` is now registered as an Whitelisted Anti Links Channel`)
                  .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
        ...
        await tempmsg.channel.awaitMessages(m => m.author.id === message.author.id, {
          ...
          .then(collected => {
            ...
            if (channel) {
              let antisettings = client.settings.get(message.guild.id, "antilink.whitelistedchannels")
              if (!antisettings.includes(channel.id)) return message.reply({embed: new Discord.MessageEmbed()
                .setTitle(`<:cross:899255798142750770>  ERROR | The Channel: \`${channel.name}\` is not added to the Anti Links Whitelist yet`)
                .setColor(es.wrongcolor)
                .setFooter(es.footertext, es.footericon)
              });
              try {
                ...
                return message.reply(new Discord.MessageEmbed()
                  .setTitle(`<:tick:899255869185855529> The Channel: \`${channel.name}\` is now **NOT** registered as an Whitelisted Anti Links Channel anymore`)
                  .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
                  .setDescription(`Every single Channel:\n<#${client.settings.get(message.guild.id, "antilink.whitelistedchannels").join(">\n<#")}>\nis not a checked by the Anti Links System`.substr(0, 2048))
                  .setFooter(es.footertext, es.footericon)
                );

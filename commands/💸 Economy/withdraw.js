  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    if(args[0].toLowerCase() == "all"){
      ...
      return message.reply(new MessageEmbed()
        ...
        .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
        .setTitle(`<:tick:899255869185855529> You withdrawed **\`${nFormatter(withdrawed)}💸\`** from your Bank`)
      ...
      return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
          .setTitle(`<:cross:899255798142750770>  You can't withdraw a negative Amount of Money or no Money, from your Bank`)
        );
      ...
      if(amount > data.bank)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
          .setTitle(`<:cross:899255798142750770>  You can't withdraw more Money than you have in your **🏦 Bank (\`${data.balance} 💸\`)**`)
        );
      ...
      return message.reply(new MessageEmbed()
        .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
        .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
        .setTitle(`<:tick:899255869185855529> You withdrawed **\`${nFormatter(amount)}💸\`** from your Bank`)
        .setDescription(`**🏦 You now have \`${nFormatter(Math.floor(data.bank))} 💸\` in your Bank**\n\n👛 You now have \`${nFormatter(Math.floor(data.balance))} 💸\` in your Pocket`)
      );
    }
  } catch (e) {

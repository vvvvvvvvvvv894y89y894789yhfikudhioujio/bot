  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    if(data.black_market.boost.time !== 0 && timeout - (Date.now() - data.black_market.boost.time) > 0){
      ...
      return message.reply({embed: new MessageEmbed()
        ...
        .setTitle(`<:cross:899255798142750770>  You've already bought a 2 Day Boost in the last 5 Days!`)
    ...
    //YEA
      ...
      if(!amount)
        return message.channel.send(new MessageEmbed()
          ...
          .setTitle(`<:cross:899255798142750770>  You did not add the Multiplier you want to buy for 2 Days!`)
      ...
      if(amount == 0 || amount < 0)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
          .setTitle(`<:cross:899255798142750770>  You cannot buy a 0 or less then 0 Multiplier, for 2 Days`)
          .setDescription(`Usage: \`${prefix}blackmarket <Multiplier>\`\n\n\Example: \`${prefix}blackmarket 3\``)
        );
      if(amount == 1)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
          .setTitle(`<:cross:899255798142750770>  You cannot buy a 1 Multiplier, for 2 Days`)
          .setDescription(`Usage: \`${prefix}blackmarket <Multiplier>\`\n\n\Example: \`${prefix}blackmarket 3\``)
        );
      ...
      if(amount > 5)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
          .setTitle(`<:cross:899255798142750770>  You cannot buy a Multiplier bigger then 5, for 2 Days`)
          .setDescription(`Usage: \`${prefix}blackmarket <Multiplier>\`\n\n\Example: \`${prefix}blackmarket 3\``)
        );
      if(prize * (amount - 1) > data.balance)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
          .setTitle(`<:cross:899255798142750770>  You can't pay more Money (\`${(amount - 1)}x ${prize}\`) than you have in your **👛 Pocket (\`${data.balance} 💸\`)**`)
        );
      ...
      return message.reply(new MessageEmbed()
        ...
        .setFooter(message.author.tag, message.author.displayAvatarURL({dynamic: true}))
        .setTitle(`<:tick:899255869185855529> You've bought a ${amount}x Boost for 2 Days, for \`${prize * (amount - 1)} 💸\`, You can buy one again in 5 Days`)

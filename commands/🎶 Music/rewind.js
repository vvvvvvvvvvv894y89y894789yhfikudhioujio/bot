  ...
  run: async (client, message, args, cmduser, text, prefix, player) => {
    ...
    try{
      ...
      if (!args[0])
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setTitle(`<:cross:899255798142750770>  You may rewind for \`1\` - \`${player.queue.current.duration}\``)
        );
      ...
      return message.channel.send(new MessageEmbed()
        .setTitle(`<:tick:899255869185855529> ${emoji.msg.rewind} Rewinded the song for \`${args[0]} Seconds\` to: ${format(Number(player.position))}`)
